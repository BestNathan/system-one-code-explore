...
- Route every online System One request through `src/system_one_client.py::SystemOneClient.call`, either directly or through the `SystemOneDecider` compatibility adapter. Do not add another HTTP transport, retry loop, raw-call logger, or usage normalizer in research modules.
